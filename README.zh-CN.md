...
### 11. 交互模式 (Interactive Mode)
- **默认开启**：交互模式默认启用，AI 会在每个决策点（复杂度评估、阶段导航、变更管理等）使用结构化选择 UI，而非纯文本提示。如需关闭，使用 `/od config interactive off`。
- **一键决策**：无需手动输入 `/od 继续` 或 `/od 跳过` 等命令，AI 会直接展示可点击的选项供你选择，减少交互摩擦，让整个对话在一个 request 中流转。
- **持久化偏好**：配置保存在 `docs/omnidev-state/config.json` 中，跨会话生效。随时可通过 `/od config interactive off` 关闭，或 `/od config interactive on` 重新开启。

### 12. 自动进化引擎 (Self-Evolution Engine)
...
| `/od cfg -i on/off` | — | 🎛️ 开启/关闭交互模式（决策点使用结构化选择 UI；默认开启） |
