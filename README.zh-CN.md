...
- **用户掌控**：所有规则变更必须经用户明确批准。使用 `/od evolve` 查看提案，可选择性采纳或拒绝。AI 绝不会静默修改自身的行为规则。
- **安全护栏**：进化引擎不能削弱核心安全规则（如 `/od` 前缀强制规则或安全护栏）。每次进化都有日志记录，可通过 `/od evolve --rollback [N]` 回滚。
...
| `/od evolve` | 🧬 自动进化：审查并应用累积的学习信号，改进规则和技能 |
| `/od evolve --review` | 🔍 查看当前进化日志和待处理提案 |
| `/od evolve --rollback [N]` | ⏪ 回滚指定的进化操作 |
| `/od resume` | 🔄 恢复上一次 OmniDev 会话（加载工具规则；建议不用裸 `/resume`） |
